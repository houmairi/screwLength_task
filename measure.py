...
def measure_screw_length(image_path, output_folder, min_length=500):
        ...
        if points.shape[0] > 2:  # Ensure we have enough points for PCA
            ...
            if len(intersections) >= 2:
                ...
                cv2.putText(img, "Measurement failed", (10, 30), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(mask_colored, "Measurement failed", (10, 30), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        else:
            length = 0
            cv2.putText(img, "Not enough points for measurement", (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            cv2.putText(mask_colored, "Not enough points for measurement", (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    else:
        length = 0
    ...
    # Create side-by-side comparison with three images
